...
#    version('9.0.X',git='https://github.com/gartung/cmssw.git',branch='CMSSW_9_0_X')
    ...
    def install(self, spec, prefix):
        ...
        values['XZ_VER']=str('6.3.0')
        values['XZ_PREFIX']=str('/usr')
        values['ZLIB_VER']=str('')
        ...
        values['PCRE_VER']=str('1.1.0')
        values['PCRE_PREFIX']=str('/usr')
        values['OPENSSL_VER']=str(spec['openssl'].version)
        values['OPENSSL_PREFIX']=str(spec['openssl'].prefix)
        values['LIBXML2_VER']=str('10.0.0')
        values['LIBXML2_PREFIX']=str('/usr')
        ...
        values['BZ2_VER']=str('1.0.0')
        values['BZ2_PREFIX']=str('/usr')
        ...
        with working_dir(join_path(build_directory,cmssw_u_version),create=False):
            ...
            for m in matches: 
                rm('-v',m,output=str)
            scram('-a', 'osx1012_amd64_clang8', 'build', 'distclean')
            scram('-a', 'osx1012_amd64_clang8', 'build', '-v', '-k','-j8', 'SCRAM_NOEDM_CHECKS=1')
